...
export const GET_BUTTON_STYLE = (btnStyle: "CTA_PRIMARY" | "CTA_SECONDARY", theme?: "LIGHT" | "DARK") => {
  switch (btnStyle) {
    case "CTA_PRIMARY":
      return {
        parentWrapper:
          "px-6 min-w-[170px] min-h-[45px] py-2.5 bg-(--cta-button-background) text-(--root-white-color) font-instrument-sans text-sm sm:text-base rounded-full border-0 outline-0  font-semibold flex items-center justify-between gap-4 group/btn overflow-hidden relative cursor-pointer ring-[1px] ring-(--cta-button-background)",
